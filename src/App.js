...
function App() {
    ...
    React.useEffect(() => {
        ...
        const pageParam = categoryId ? `category=${categoryId}` : "";


        fetch(`https://64930483428c3d2035d125ea.mockapi.io/photos?page=${page}&limit=3&${category}`)
            .then(res => res.json())
            .then(json => setCollections(json))
            .catch(err => {
                console.warn(err);
                alert('Оштбка при получении данных');
            })
            .finally(() => {
                setIsLoading(false);
            })
